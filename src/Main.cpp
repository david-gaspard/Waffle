...
/**
 * @todo TODO:
 * (1) In WaveSystem: If possible, compute the exact expression of the free DOS on a square lattice. It is used in setDisorder()...
 * (2) In Main: Add histogram of transmission eigenvalues...
 * (3) In Main: Add parallelized taskTransmissionOMP()...
 * (4) In plot_map.py: Fix the partial read bug with the CSV reader which occurs when it is called by the C++ program...
 * (5) In SquareMesh: Create addImage(filename) to import PNG in order to facilitate the mesh creation...
 */
