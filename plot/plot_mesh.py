...
def boundary_to_tikz_code(data, epsilon):
    """
    ...
    epsilon = Tolerance of the Ramer-Douglas-Peucker algorithm (value eps=1.3 is ok, epsilon=0 to disable).
    """
...
def plot_mesh(args):
    ...
    try:
        fp = open(args.mesh_file, 'r')
    except IOError as e:
        print(ct.TAG_ERROR + "Field file '" + args.mesh_file + " not found, aborting now...")
        return 1
...
\\end{{tikzpicture}}%""".format(
        ...
        boundary_code = boundary_to_tikz_code(data, args.epsilon)
    )
    
    ## Export the TikZ code to a file and compile it:
    file_path = os.path.splitext(args.mesh_file)[0]  ## The file path is the filename without its extension (used to write new files). 
...
def parseArguments():
    ...
    parser.add_argument("mesh_file", type=str, help="Full file path of the mesh file.")
